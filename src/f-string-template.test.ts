...
describe('FStringTemplate', () => {
  ...
  it('should get registered PromptTemplate', () => {
    ...
    expect(
      new StringTemplate('{text}', { templateFormat: 'fstring' })
    ).toBeInstanceOf(FStringTemplate)
  })

  it('should pass PromptTemplate to value', async () => {
    const systemTemplate = FStringTemplate.from(
  ...
  it('should format text without required variables', async () => {
    expect(await FStringTemplate.from(`hi{text}`).format({})).toStrictEqual(
      'hi'
    )
  })
  ...
  it('should format text with required variables', async () => {
    expect(
      await FStringTemplate.from(`hi {text}`).format({ text: 'world' })
    ).toStrictEqual('hi world')
    expect(
      await FStringTemplate.from(`hi {text}`, {
        data: { text: 'world' },
      }).format()
    ).toStrictEqual('hi world')
  })
  ...
  it('should format directly by PromptTemplate', async () => {
    expect(
      await StringTemplate.format({
        template: '{  text } world {good}!',
        data: { text: 'hello', good: 'better' },
        templateFormat: 'langchain',
      })
    ).toStrictEqual('hello world better!')
  })
  ...
  it('should return raw value for pure placeholder with raw option', async () => {
    ...
    const result = await FStringTemplate.from('{obj}', { raw: true }).format(
      data
    )
    expect(result).toStrictEqual(data.obj)
  })

  describe('isTemplate', () => {
    it('should test isTemplate true', () => {
      expect(
        FStringTemplate.isTemplate({ template: '{strings}: {a} + {b}' })
      ).toBeTruthy()
    })

    it('should test isTemplate false', () => {
      expect(
        FStringTemplate.isTemplate({ template: 'a {strings ' })
      ).toBeFalsy()
    })
  ...
  describe('matchTemplateSegment', () => {
    it('should test matchTemplateSegment', () => {
      ...
      let result = FStringTemplate.matchTemplateSegment({
      ...
      result = FStringTemplate.matchTemplateSegment(
        { template: templateStr },
        pos
      )
      expect(result).toBeDefined()
      ...
      expect(
        FStringTemplate.matchTemplateSegment({ template: 'a {strings ' })
      ).toBeUndefined()
    })
    ...
    it('should test matchTemplateSegment by StringTemplate', () => {
      ...
      result = StringTemplate.matchTemplateSegment(
        { template: templateStr, templateFormat: 'langchain' },
        pos
      )
  ...
  describe('getPurePlaceholderVariable', () => {
    it('should extract variable from pure placeholders', () => {
      expect(FStringTemplate.getPurePlaceholderVariable('{var}')).toBe('var')

      expect(FStringTemplate.getPurePlaceholderVariable('  { var }  ')).toBe(
        ' var '
      )
    ...
    it('should return undefined for mixed content or escaped braces', () => {
      expect(
        FStringTemplate.getPurePlaceholderVariable('Hello {var}')
      ).toBeUndefined()

      expect(FStringTemplate.getPurePlaceholderVariable('{{')).toBeUndefined()
    })
  })
})
