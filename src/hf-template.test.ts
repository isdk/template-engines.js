...
describe('HfStringTemplate', () => {
  it('should use global env', async () => {
    ...
    expect(await HfStringTemplate.from('{{say()}}').format()).toStrictEqual(
      'hello'
    )
    HFTemplate.global.clear()
  })
  it('should get inputVariables from template', () => {
    let template = HfStringTemplate.from(
      `{% for s in strings[:] %}{{ s }}{% endfor %} {{strings}} {{a+b}}`
    )
    expect(template.inputVariables).toStrictEqual(['strings', 'a', 'b'])
    template = HfStringTemplate.from(
      `{% if aa == 2 %}{{ aa }}{% else %}{{ bb }}{% endif %}{{ cc }}`
    )
    expect(template.inputVariables).toStrictEqual(['aa', 'bb', 'cc'])
    template = HfStringTemplate.from(
      `{% set aa = 2 %} {% if aa == 2 %}{{ aa }}{% else %}{{ bb }}{% endif %}{{ cc }}`
    )
  ...
  it('should format text without required variables', async () => {
    expect(await HfStringTemplate.from(`hi{{text}}`).format({})).toStrictEqual(
      'hi'
    )
  })
  ...
  it('should format text with required variables', async () => {
    expect(
      await HfStringTemplate.from(`hi {{text}}`).format({ text: 'world' })
    ).toStrictEqual('hi world')
    expect(
      await HfStringTemplate.from(`hi {{text}}`, {
        data: { text: 'world' },
      }).format()
    ).toStrictEqual('hi world')
  })
    ...
    expect(
      await StringTemplate.format({
        ...
        data: {
          ...
          x: 1,
          apple: 'pear',
          func: createHfValueFunc((content: any) =>
            Object.entries(content).flat().join(' ')
          ),
        },
      })
  ...
  describe('isTemplate', () => {
    it('should test isTemplate true', () => {
      expect(
        HfStringTemplate.isTemplate({ template: '{{ strings }}: {{a}} + {b}' })
      ).toBeTruthy()
      expect(
        HfStringTemplate.isTemplate({ template: '{{ "hello world" }}' })
      ).toBeTruthy()
      expect(
        HfStringTemplate.isTemplate({ template: '{% if true %}hi{%endif%}' })
      ).toBeTruthy()
    })

    it('should test isTemplate false', () => {
      expect(
        HfStringTemplate.isTemplate({ template: 'Hi world\n' })
      ).toBeFalsy()
      expect(
        HfStringTemplate.isTemplate({ template: 'a {{strings ' })
      ).toBeFalsy()
      expect(
        HfStringTemplate.isTemplate({ template: 'a {{strings }' })
      ).toBeFalsy()
    ...
    it('should test isTemplate directly by PromptTemplate true', async () => {
      expect(
        StringTemplate.isTemplate({ template: '{{text}} world' })
      ).toBeTruthy()
    })
  ...
  describe('matchTemplateSegment', () => {
    it('should test matchTemplateSegment', () => {
      ...
      let result = HfStringTemplate.matchTemplateSegment({
      ...
      result = HfStringTemplate.matchTemplateSegment(
        { template: templateStr },
        pos
      )
      expect(result).toBeDefined()
      ...
      expect(
        HfStringTemplate.matchTemplateSegment({ template: 'a {{strings ' })
      ).toBeUndefined()
    })
    ...
    it('should test matchTemplateSegment by StringTemplate', () => {
      ...
      expect(result).toBeUndefined()
      expect(
        StringTemplate.matchTemplateSegment({ template: 'a {{strings ' })
      ).toBeUndefined()
    })
  })
  ...
  describe('getPurePlaceholderVariable', () => {
    it('should extract variable name from pure placeholders', () => {
      expect(HfStringTemplate.getPurePlaceholderVariable('{{ var }}')).toBe(
        'var'
      )
      expect(
        HfStringTemplate.getPurePlaceholderVariable('  {{  var.name  }}  ')
      ).toBe('var.name')
      expect(
    ...
    it('should return undefined for statements or mixed content', () => {
      expect(
        HfStringTemplate.getPurePlaceholderVariable('{% if true %}')
      ).toBeUndefined()
      expect(
