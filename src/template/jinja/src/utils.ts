...
export function slice<T>(
  ...
  if (direction >= 0) {
    ...
    stop =
      (stop ??= array.length) < 0
        ? Math.max(array.length + stop, 0)
        : Math.min(stop, array.length)
  } else {
    start =
      (start ??= array.length - 1) < 0
        ? Math.max(array.length + start, -1)
        : Math.min(start, array.length - 1)
    stop =
      (stop ??= -1) < -1
        ? Math.max(array.length + stop, -1)
        : Math.min(stop, array.length - 1)
  }
