...
export function parse(tokens: Token[]): Program {
    ...
    switch (tokens[current].type) {
      ...
      default:
        throw new SyntaxError(
          `Unknown statement type: ${tokens[current].type} ${tokens[current].value}`
        )
    }
  ...
  function parseForStatement(): For {
    ...
    while (
      not(TOKEN_TYPES.OpenStatement, TOKEN_TYPES.EndFor) &&
      not(TOKEN_TYPES.OpenStatement, TOKEN_TYPES.Else)
    ) {
      body.push(parseAny())
    }
