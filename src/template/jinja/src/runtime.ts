  ...
  override builtins = new Map<string, AnyRuntimeValue>([
    ...
    [
      ...
      new FunctionValue((args) => {
        ...
        if (!(searchString instanceof StringValue)) {
          throw new TypeError('startswith() argument must be a string')
        }
        const position = args[1]
        if (position && !(position instanceof NumericValue)) {
          throw new TypeError(
            'startswith() argument 2 must be an integer or None for position'
          )
        }
        return new BooleanValue(
          this.value.startsWith(searchString.value, position?.value)
        )
      }),
    ],
    [
      ...
      new FunctionValue((args) => {
        ...
        return new BooleanValue(
          this.value.endsWith(searchString.value, position?.value)
        )
      }),
    ],
  ])
...
export class ObjectValue extends RuntimeValue<Map<string, AnyRuntimeValue>> {
  ...
  constructor(
    value: Map<string, AnyRuntimeValue>,
    public orgValue?: Record<string, any>
  ) {
    super(value)
  }
  ...
  override builtins: Map<string, AnyRuntimeValue> = new Map<
    string,
    AnyRuntimeValue
  >([
    [
...
export class FunctionValue extends RuntimeValue<
  (args: AnyRuntimeValue[], scope: Environment) => AnyRuntimeValue
> {
...
export class Environment {
  ...
  tests: Map<string, (...value: AnyRuntimeValue[]) => boolean> = new Map([
    ...
    [
      'false',
      (operand) =>
        operand.type === 'BooleanValue' && !(operand as BooleanValue).value,
    ],
    [
      'true',
      (operand) =>
    ...
    [
      'integer',
      (operand) =>
        operand.type === 'NumericValue' &&
        Number.isInteger((operand as NumericValue).value),
    ],
    [
      'iterable',
      (operand) =>
        operand instanceof ArrayValue || operand instanceof StringValue,
    ],
    [
      'lower',
      (operand) => {
...
export class Interpreter {
  ...
  private evaluateBinaryExpression(
    ...
    if (left instanceof UndefinedValue || right instanceof UndefinedValue) {
      ...
      const member =
        right.value.find((x) => x.value === left.value) !== undefined
      switch (node.operator.value) {
        case 'in':
        ...
        switch (filter.value) {
          ...
          case 'capitalize':
            return new StringValue(
              operand.value.charAt(0).toUpperCase() + operand.value.slice(1)
            )
          case 'trim':
      ...
      if (operand instanceof ArrayValue) {
        ...
        switch (filterName) {
          case 'indent': {
            ...
            const width =
              args.at(0) ?? kwargs.get('width') ?? new NumericValue(4)
            if (!(width instanceof NumericValue)) {
              throw new Error('width must be a number')
            }
            const first =
              args.at(1) ?? kwargs.get('first') ?? new BooleanValue(false)
            const blank =
              args.at(2) ?? kwargs.get('blank') ?? new BooleanValue(false)
  ...
  private evaluateBlock(
    ...
    for (const statement of statements) {
      ...
      if (
        lastEvaluated.type !== 'NullValue' &&
        lastEvaluated.type !== 'UndefinedValue'
      ) {
  ...
  private evaluateSet(node: SetStatement, environment: Environment): NullValue {
    ...
    if (node.assignee.type === 'Identifier') {
      ...
      throw new Error(
        `Invalid LHS inside assignment expression: ${JSON.stringify(node.assignee)}`
      )
    }
  ...
  private evaluateFor(node: For, environment: Environment): StringValue {
    ...
    if (!(iterable instanceof ArrayValue)) {
      throw new Error(
        `Expected iterable type in for loop: got ${iterable.type}`
      )
    }
    ...
    for (let i = 0; i < iterable.value.length; ++i) {
      ...
      if (node.loopvar.type === 'Identifier') {
        ...
        if (loopvar.value.length !== c.value.length) {
          throw new Error(
            `Too ${loopvar.value.length > c.value.length ? 'few' : 'many'} items to unpack`
          )
        }
...
function parseRuntimeValue(
  arg: AnyRuntimeValue | Map<string, any> | Array<any>
) {
  let result: any = arg
  if (Array.isArray(arg)) {
    ...
    switch (arg.type) {
      ...
      case 'UndefinedValue': // JSON.stringify(undefined) -> undefined
        ...
        break
      case 'ArrayValue':
        result = (arg as ArrayValue).value.map((item: AnyRuntimeValue) =>
          parseRuntimeValue(item)
        )
        break
