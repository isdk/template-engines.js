...
describe('Golang Template', () => {
  ...
  it('should render variable template', () => {
    const result = interpolateGolangTemplate(`{{.strings}}: {{.a}} {{.b}}`, {
      strings: 'jack',
      a: 'hello',
      b: 'world',
    })
  ...
  it('should render if-else template', () => {
    ...
    let result: any = interpolateGolangTemplate(template, { color: 'Red' })
    ...
    result = interpolateGolangTemplate(
      'Is color defined? {{if .color}}Yes{{end}}{{if .color}}Yes{{end}}',
      { color: 'Red' }
    )
    expect(result).toStrictEqual('Is color defined? YesYes')
    result = interpolateGolangTemplate(
      'Is color defined? {{if .color}}Yes{{end}}',
      {}
    )
  ...
  it('should render reReplace template', () => {
    ...
    expect(result).toStrictEqual(
      'Original text: An apple a day keeps the doctor away., Replaced text: An orange a day keeps the doctor away.An pear a day keeps the doctor away.'
    )
    result = interpolateGolangTemplate(
      `Original text: {{.text}}, Replaced text: {{re_replace .text "apple" 'orange'}}`,
      { text: 'An apple a day keeps the doctor away.' }
    )
    expect(result).toStrictEqual(
      'Original text: An apple a day keeps the doctor away., Replaced text: An orange a day keeps the doctor away.'
    )
    result = interpolateGolangTemplate(
      `Original text: {{.text}}, Replaced text: {{re_replace .text 'apple" 'orange'}}`,
      { text: 'An apple a day keeps the doctor away.' }
    ...
    expect(result).toStrictEqual(
      `Original text: An apple a day keeps the doctor away., Replaced text: {{re_replace .text 'apple" 'orange'}}`
    )
    result = {}
    interpolateGolangTemplate(template, result, true)
